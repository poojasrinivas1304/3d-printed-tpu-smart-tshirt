...
12. **Reproducibility details.** The documentation identifies the author-confirmed materials as Conductive Filaflex from Recreus and a size-XL Zoofly polyester T-shirt. The exact fibre percentage and textile-construction specifications were not available. The manuscript reports garment fit, participant characteristics and eligibility criteria, software versions, recording-level baseline normalization, and whether protocol boundaries are required. The earlier statement that the filament manufacturer/product was unknown has been superseded by the author's confirmation.
...
The reproducibility repository contains the acquisition firmware, grouped nested-validation code, fold-level sensor selections and metrics, participant- and posture-level metrics, ADC quality-control summaries, and analytical figure-generation scripts. Per-window predictions (`outer_fold_predictions.csv`) and the grouped Figure 9 (`Fig9_grouped_nested.png`) are generated locally when the Python analysis is run; they are not committed files. See the [reproduction instructions](../results/nested_grouped/README.md#reproduction).
