...
`position_index=4` / `touch_left_shoulder` means right hand touching the left shoulder with a leftward torso twist. `position_index=5` / `touch_right_shoulder` means left hand touching the right shoulder with a rightward torso twist. Directions refer to the participant.  See [movement-label reconciliation](../../docs/movement_label_reconciliation.md).
